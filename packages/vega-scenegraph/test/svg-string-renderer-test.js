...
function render(scene, w, h) {
  vega.resetSVGDefIds();
  return new Renderer().initialize(null, w, h).render(scene).svg();
}
...
tape(
  "SVGStringRenderer should build empty group for item-less area mark",
  (t) => {
...
tape(
  "SVGStringRenderer should build empty group for item-less line mark",
  (t) => {
    const r = new Renderer();
