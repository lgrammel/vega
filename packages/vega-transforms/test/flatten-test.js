...
tape('Flatten flattens arrays', t => {
  ...
  var fl = df.add(Flatten, {fields: [v], pulse: c0});
  var out = df.add(Collect, {pulse: fl});
  var d;
...
tape('Flatten flattens parallel arrays', t => {
  ...
  var df = new vega.Dataflow();
  var c0 = df.add(Collect);
  var fl = df.add(Flatten, {fields: [a, b], pulse: c0});
  var out = df.add(Collect, {pulse: fl});
  var d;
...
tape('Flatten flattens and adds index field', t => {
  ...
  var df = new vega.Dataflow();
  var c0 = df.add(Collect);
  var fl = df.add(Flatten, {fields: [v], pulse: c0, index: 'foo'});
  var out = df.add(Collect, {pulse: fl});
  var d;
