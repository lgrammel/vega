...
tape("Facet facets tuples", (t) => {
  ...
  df.pulse(source, changeset().modify(data[3], "k", "c")).run();
  ...
  df.pulse(source, changeset().remove([data[2], data[3], data[4]])).run();
  t.equal(facet.targets().active, 2); // 2 subflows updated
  t.equal(subs.length, 3); // no new subflows added
  ...
  df.pulse(
    source,
    changeset().remove([data[0], data[1], data[5]]).clean(false) // don't remove subflows
  ).run();
  t.equal(facet.targets().active, 3); // 3 subflows updated
  t.equal(subs.length, 3); // no new subflows added
  ...
  df.pulse(
    source,
    changeset().remove(truthy).clean(true) // remove subflows from internal map
  ).run();
