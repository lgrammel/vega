...
export const AggregateOps = {
  ...
  mean: {
    ...
    rem: (m, v) => (
      (m.mean_d = v - m.mean), (m.mean -= m.valid ? m.mean_d / m.valid : m.mean)
    ),
  },
  average: {
    value: (m) => (m.valid ? m.mean : undefined),
  ...
  stderr: {
    value: (m) =>
      m.valid > 1 ? Math.sqrt(m.dev / (m.valid * (m.valid - 1))) : undefined,
    req: ["variance"],
    idx: 2,
  },
  distinct: {
  ...
  min: {
    ...
    value: (m) =>
      (m.min = Number.isNaN(m.min) ? m.cell.data.min(m.get) : m.min),
    add: (m, v) => {
  ...
  max: {
    ...
    value: (m) =>
      (m.max = Number.isNaN(m.max) ? m.cell.data.max(m.get) : m.max),
    add: (m, v) => {
