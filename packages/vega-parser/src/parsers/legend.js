...
import legendSymbolGroups, {
  legendSymbolLayout,
} from "./guides/legend-symbol-groups";
...
export default function (spec, scope) {
  ...
  LegendScales.forEach((s) =>
    spec[s] ? ((scales[s] = spec[s]), (scale = scale || spec[s])) : 0
  );
  ...
  if (type === Gradient) {
    ...
    params.count =
      params.count ||
      scope.signalRef(`max(2,2*floor((${deref(_.gradientLength())})/100))`);
  }
  ...
  return parseMark(
    guideGroup({
      ...
      from: dataRef,
      encode: extendEncode(
        buildLegendEncode(_, spec, config),
        legendEncode,
        Skip
      ),
      marks: children,
...
function legendType(spec, scaleType) {
  ...
  if (!spec.type && scaleCount(spec) === 1 && (spec.fill || spec.stroke)) {
    type = isContinuous(scaleType)
      ? Gradient
      : isDiscretizing(scaleType)
      ? Discrete
      : Symbols;
  }
