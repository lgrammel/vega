...
export const getSign = (orient, a, b) =>
  isSignal(orient)
...
export const ifX = (orient, a, b) =>
  isSignal(orient) ? ifXEnc(orient.signal, a, b) : isX(orient) ? a : b;

// condition on axis y-direction
export const ifY = (orient, a, b) =>
  isSignal(orient) ? ifYEnc(orient.signal, a, b) : isX(orient) ? b : a;

export const ifTop = (orient, a, b) =>
  isSignal(orient)
...
export const ifRight = (orient, a, b) =>
  isSignal(orient)
...
const ifXEnc = ($orient, a, b) =>
  ifEnc(`${$orient} === '${Top}' || ${$orient} === '${Bottom}'`, a, b);

const ifYEnc = ($orient, a, b) =>
  ifEnc(`${$orient} !== '${Top}' && ${$orient} !== '${Bottom}'`, a, b);

const ifLeftTopExpr = ($orient, a, b) =>
  ifExpr(`${$orient} === '${Left}' || ${$orient} === '${Top}'`, a, b);

const ifTopExpr = ($orient, a, b) => ifExpr(`${$orient} === '${Top}'`, a, b);

const ifRightExpr = ($orient, a, b) =>
  ifExpr(`${$orient} === '${Right}'`, a, b);
...
const toExpr = (v) =>
  isSignal(v) ? v.signal : v == null ? null : stringValue(v);

export const mult = (sign, value) =>
  value === 0
    ? 0
    : isSignal(sign)
