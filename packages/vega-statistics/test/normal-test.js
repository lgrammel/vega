...
tape("cumulativeNormal matches R output", (t) => {
  var v = [-3, -2, -1, 0, 1, 2, 3],
    R = [
      0.001349898, 0.022750132, 0.158655254, 0.5, 0.841344746, 0.977249868,
      0.998650102,
    ];

  v.map((x) => stats.cumulativeNormal(x)).forEach((x, i) =>
...
tape("densityNormal matches R output", (t) => {
  var v = [-3, -2, -1, 0, 1, 2, 3],
    R = [
      0.004431848, 0.053990967, 0.241970725, 0.39894228, 0.241970725,
      0.053990967, 0.004431848,
    ];

  v.map((x) => stats.densityNormal(x)).forEach((x, i) =>
    closeTo(t, x, R[i], 1e-3)
  );
...
tape("quantileNormal matches R output", (t) => {
  var p = [0.95, 0.9, 0.75, 0.5, 0.25, 0.1, 0.05],
    R = [
      1.6448536, 1.2815516, 0.6744898, 0.0, -0.6744898, -1.2815516, -1.6448536,
    ];

  p.map((x) => stats.quantileNormal(x)).forEach((x, i) =>
    closeTo(t, x, R[i], 1e-3)
  );
