...
tape("JSON reader should parse date with format %d.%m.%Y", (t) => {
  var expected = function () {
      return [{ foo: new Date(1990, 6, 18) }];
    },
    input = function () {
      return [{ foo: "18.07.1990" }];
    },
    types;
...
tape("JSON reader should parse time with format %H:%M", (t) => {
  var expected = function () {
      return [{ foo: new Date(1900, 0, 1, 13, 15) }];
    },
    input = function () {
      return [{ foo: "13:15" }];
    },
...
tape("JSON reader should parse UTC date with format %d.%m.%Y", (t) => {
  var expected = function () {
      return [{ foo: new Date(Date.UTC(1990, 6, 18)) }];
    },
    input = function () {
      return [{ foo: "18.07.1990" }];
    },
...
tape("JSON reader should parse UTC time with format %H:%M", (t) => {
  var expected = function () {
      return [{ foo: new Date(Date.UTC(1900, 0, 1, 13, 15)) }];
    },
    input = function () {
      return [{ foo: "13:15" }];
    },
...
tape("JSON reader should throw error if format is unrecognized", (t) => {
  ...
  t.throws(() => {
    read(input, { type: "json", parse: types });
  });
  t.end();
});
...
tape("DSV reader should auto-parse dsv fields", (t) => {
  t.deepEqual(
    read(psv, { type: "dsv", delimiter: "|", parse: "auto" }),
    parsed
  );
  t.end();
});
...
const worldText = require("fs").readFileSync(
  "./test/data/world-110m.json",
  "utf8"
);
...
tape("TopoJSON reader should read TopoJSON mesh interior", (t) => {
  var mesh = read(worldText, {
    type: "topojson",
    mesh: "countries",
    filter: "interior",
  });
  const tj = topojson.mesh(
    world,
...
tape(
  "TopoJSON reader should throw error if TopoJSON parameters are missing",
  (t) => {
    t.throws(() => {
      read(worldText, { type: "topojson" });
    });
    t.end();
  }
);
