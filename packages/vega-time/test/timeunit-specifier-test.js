...
tape("timeUnitSpecifier produces specifier for multiple time units", (t) => {
  ...
  t.equal(
    vega.timeUnitSpecifier(["year", "month", "date", "hours"]),
    "%Y-%m-%d %H:00"
  );
  t.equal(
    vega.timeUnitSpecifier(["year", "month", "date", "minutes"]),
    "%Y-%m-%d 00:%M"
  );
  t.equal(
    vega.timeUnitSpecifier(["year", "month", "date", "hours", "minutes"]),
    "%Y-%m-%d %H:%M"
  );
  t.equal(
    vega.timeUnitSpecifier(["hours", "minutes", "seconds", "milliseconds"]),
    "%H:%M:%S.%L"
  );
  t.end();
});
