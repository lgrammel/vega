...
function floor(unit, date) {
  switch (unit) {
    case "year":
      return (d) => date(d.y, 0, 1);
    case "quarter":
      return (d) => date(2012, 3 * d.q, 1);
    case "month":
      return (d) => date(2012, d.m, 1);
    case "week":
      return (d) => date(2012, 0, 7 * (d.w - 1) + 1);
    case "date":
      return (d) => date(2012, 0, d.d);
    case "day":
      return (d) => date(2012, 0, d.u + 1);
    case "dayofyear":
      return (d) => date(2012, 0, d.doy);
    case "year-quarter":
      return (d) => date(d.y, 3 * d.q, 1);
    case "year-month":
      return (d) => date(d.y, d.m, 1);
    case "year-month-date":
      return (d) => date(d.y, d.m, d.d);
    case "year-week":
      return (d) => date(d.y, 0, 7 * (d.w - 1) + 1);
    case "year-week-day":
      return (d) => date(d.y, d.m, d.d);
    case "year-dayofyear":
      return (d) => date(d.y, 0, d.doy);
    case "month-date":
      return (d) => date(2012, d.m, d.d);
    case "week-day":
      return (d) => date(2012, 0, 7 * (d.w - 1) + d.u + 1);
  }
}
...
tape("timeFloor generates local floor function", (t) => {
  ...
  UNITS.forEach((u) =>
    testFloor(t, data, vega.timeFloor(u.split("-")), floor(u, local))
  );
...
tape("utcFloor generates utc floor function", (t) => {
  ...
  UNITS.forEach((u) =>
    testFloor(t, data, vega.utcFloor(u.split("-")), floor(u, utc))
  );
